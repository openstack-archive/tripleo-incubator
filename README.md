...
Caveats/Limitations:
 ...
 - Currently assumes two clouds: under cloud and over cloud. Long term we would
   like to be able to offer a single cloud for encironments where that makes
...
1. Developer tools (like disk-image-builder) should have a non-intrusive
   ...
   is poor form.
1. Where possible we run the code developer wrote - e.g. we strongly prefer
   to use Nova default settings rather than our own. Only if there is no
   right setting in production should not attempt this.
1. We only prototype tools in tripleo-incubator: when the are ready for
...
metadata is typically not cachable or highly mutable, and a local mirror will
be a big timesaver.

Also always setup a local pypi mirror - either with pypi-mirror (and we have
instructions for in the diskimage-builder ``pypi`` element README.md).

Run small steps - TripleO is composed of small composable tools. Do not use
``devtest.sh`` as the entry point for development - its a full run of the
...
core tools directly - thats what they are for.

Iterating on in-instance code
-----------------------------

There are broadly three sets of code for TripleO - the heat templates, the
code that runs within instances, and code that runs after a deploy to customise
it.
...
different metadata mappings without waiting for full intial deployments to take
effect.
