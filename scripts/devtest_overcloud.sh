...
OVERCLOUD_CONTROL_DIB_ELEMENTS=${OVERCLOUD_CONTROL_DIB_ELEMENTS:-'ntp hosts baremetal boot-stack cinder-api ceilometer-collector ceilometer-api ceilometer-agent-central ceilometer-agent-notification ceilometer-alarm-notifier ceilometer-alarm-evaluator os-collect-config neutron-network-node dhcp-all-interfaces swift-proxy swift-storage keepalived haproxy sysctl'}
...
OVERCLOUD_CEPHSTORAGE_DIB_ELEMENTS=${OVERCLOUD_CEPHSTORAGE_DIB_ELEMENTS:-'ntp hosts baremetal os-collect-config dhcp-all-interfaces sysctl'}
...
if [ "${USE_CEPH:-}" = 1 ] ; then
    OVERCLOUD_CONTROL_DIB_EXTRA_ARGS="$OVERCLOUD_CONTROL_DIB_EXTRA_ARGS ceph-mon cinder-volume-ceph"
    OVERCLOUD_COMPUTE_DIB_EXTRA_ARGS="$OVERCLOUD_COMPUTE_DIB_EXTRA_ARGS nova-compute-volume-ceph"
    OVERCLOUD_CEPHSTORAGE_DIB_EXTRA_ARGS="$OVERCLOUD_CEPHSTORAGE_DIB_EXTRA_ARGS ceph-osd"
fi
...
if [ $OVERCLOUD_CEPHSTORAGESCALE -gt 0 ]; then
    if [ ! -e $TRIPLEO_ROOT/overcloud-ceph-storage.qcow2 -o "$USE_CACHE" == "0" ]; then #nodocs
        $TRIPLEO_ROOT/diskimage-builder/bin/disk-image-create $NODE_DIST \
            -a $NODE_ARCH -o $TRIPLEO_ROOT/overcloud-ceph-storage \
            $OVERCLOUD_CEPHSTORAGE_DIB_ELEMENTS $DIB_COMMON_ELEMENTS \
...
if (neutron net-list | grep -q public); then
    ...
    export CONTROLEXTRA="${CONTROLEXTRA:-} overcloud-vlan-port.yaml"
else
    VLAN_ID=
...
function create_cephx_key() {
    # Taken from https://github.com/ceph/ceph-deploy/blob/master/ceph_deploy/new.py#L21
