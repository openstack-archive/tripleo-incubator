...
wait_for -w --timeout $((60 * $expected_nodes)) --delay 10 wait_for_hypervisor_stats $expected_nodes
...
wait_for_stack_ready -w $(($OVERCLOUD_STACK_TIMEOUT * 60 / 10)) 10 $STACKNAME
...
wait_for -w --timeout 300 --delay 10 -- nova service-list --binary nova-compute 2\>/dev/null \| grep 'enabled.*\ up\ '

## #. Wait for L2 Agent On Nova Compute
##    ::

wait_for -w --timeout 300 --delay 10 -- neutron agent-list -f csv -c alive -c agent_type -c host \| grep "\":-).*Open vSwitch agent.*-novacompute\"" #nodocs
...
##    ::
    ...
    wait_for -w --timeout 50 --dealy 5 -- neutron port-list -f csv -c id --quote none \| grep id
...
wait_for -w --timeout 300 --delay 10 -- ping -c 1 $FLOATINGIP
