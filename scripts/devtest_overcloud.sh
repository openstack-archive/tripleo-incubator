...
if [ ! -e $TRIPLEO_ROOT/overcloud-control${IMAGE_TYPE:-.qcow2} -o "$USE_CACHE" == "0" ] ; then #nodocs
...
if [ ! -e $TRIPLEO_ROOT/overcloud-compute${IMAGE_TYPE:-.qcow2} -o "$USE_CACHE" == "0" ] ; then #nodocs
    $TRIPLEO_ROOT/diskimage-builder/bin/disk-image-create $NODE_DIST \
...
if [ ! -e $TRIPLEO_ROOT/user${IMAGE_TYPE:-.qcow2} -o "$USE_CACHE" == "0" ] ; then #nodocs
