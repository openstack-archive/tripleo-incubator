...
if [ ! -e $TRIPLEO_ROOT/overcloud-control.qcow2 -o "$USE_CACHE" == "0" ] ; then #nodocs
    $TRIPLEO_ROOT/diskimage-builder/bin/disk-image-create $NODE_DIST \
        ...
        baremetal boot-stack cinder-api cinder-volume os-collect-config \
...
if [ ! -e $TRIPLEO_ROOT/overcloud-compute.qcow2 -o "$USE_CACHE" == "0" ] ; then #nodocs
    $TRIPLEO_ROOT/diskimage-builder/bin/disk-image-create $NODE_DIST \
        ...
        baremetal nova-compute nova-kvm neutron-openvswitch-agent \
        os-collect-config dhcp-all-interfaces $DIB_COMMON_ELEMENTS \
        ${OVERCLOUD_DIB_EXTRA_ARGS:-} 2>&1 | \
