...
TEMP=$(getopt -o c,h -l build-only,no-mergepy,debug-logging,heat-env:,compute-flavor:,control-flavor:,cinder-storage-flavor:,swift-storage-flavor:,ceph-storage-flavor:,help -n $SCRIPT_NAME -- "$@")
...
##    ``$OVERCLOUD_*_DIB_EXTRA_ARGS`` (CONTROL, COMPUTE, ...) are
...
if [ ! -e $TRIPLEO_ROOT/overcloud-control.qcow2 -o "$USE_CACHE" == "0" ] ; then #nodocs
    $TRIPLEO_ROOT/diskimage-builder/bin/disk-image-create $NODE_DIST \
        ...
        swift-proxy swift-storage keepalived haproxy ceph-mon \
...
## #. Create your coompute image. This is the image the undercloud
...
expected_nodes=$(( $OVERCLOUD_COMPUTESCALE + $OVERCLOUD_CONTROLSCALE + $OVERCLOUD_CINDERSTORAGESCALE + $OVERCLOUD_CEPHSTORAGESCALE ))
...
if [ $OVERCLOUD_CEPHSTORAGESCALE -gt 0 ]; then
    FSID=`uuidgen`
    # Weird Ceph keys https://github.com/ceph/ceph-deploy/blob/15b348e502128df435f8c2c7073e8b936086e325/ceph_deploy/new.py#L21
