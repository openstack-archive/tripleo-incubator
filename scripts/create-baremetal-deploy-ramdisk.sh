...
OUTPUT_IMAGE_FILE=${OUTPUT_IMAGE_FILE:-ubuntu_bm_glance_deploy_image.img}
...
sudo chroot $TMP_BUILD_DIR apt-get -y install linux-image-$KERNEL_VER vlan open-iscsi glance glance-api glance-client glance-common glance-registry python-glance
...
sudo touch $TMP_BUILD_DIR/etc/init/cloud-init-nonet.conf
sudo chmod 777 $TMP_BUILD_DIR/etc/init/cloud-init-nonet.conf
...
sudo chmod 644 $TMP_BUILD_DIR/etc/init/cloud-init-nonet.conf
...
# Now Recreate the file we just removed
sudo touch $TMP_BUILD_DIR/etc/init/failsafe.conf
sudo chmod 777 $TMP_BUILD_DIR/etc/init/failsafe.conf
...
sudo chmod 644 $TMP_BUILD_DIR/etc/init/failsafe.conf
...
sudo ln -sf ../run/resolvconf/resolv.conf $TMP_BUILD_DIR/etc/resolv.conf

# oh ya don't want to forget to unmount the image
sudo umount $TMP_BUILD_DIR/dev
#sleep 30s
# now lets make a really big initrd file.
# TODO: optimize this image
cd "$TMP_BUILD_DIR"
sudo chown root:root -r *
sudo find . | sudo cpio -o -H newc | gzip > "$CURRENT_PATH/$OUTPUT_IMAGE_FILE.initrd.img"
cd $CURRENT_PATH
sudo umount $TMP_BUILD_DIR
#echo "Image file $CURRENT_PATH/$OUTPUT_IMAGE_FILE created..."
# note you may wish to keep this image
rm $CURRENT_PATH/$OUTPUT_IMAGE_FILE
