...
if [[ ! -x /.dockerinit ]]; then
    ...
    declare image="mmckeen/opensuse-13-1"
    ...
    [[ $(ip -o -4 addr show dev $bridge) =~ $bridge_addr_re ]] && \
        declare bridge_ip="${BASH_REMATCH[1]}"
...
if [[ ${1-''} = --no-devtest ]]; then
    declare SKIP_DEVTEST=true
    shift
fi
export LANG=en_US.UTF-8
export LIBVIRT_DEFAULT_URI="qemu:///system"
export LIBVIRT_DISK_BUS_TYPE="ide"
