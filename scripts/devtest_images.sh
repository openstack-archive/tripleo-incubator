...
function show_options () {
    ...
    echo "      -s                   -- Build overcloud with SSL endpoints."
...
while true ; do
    case "$1" in
        ...
        --seed|--undercloud|--overcloud-control|--overcloud-compute|--user)
            ...
            BUILD_IMAGES_SELECTED="$BUILD_IMAGES_SELECTED ${IMAGE_SELECTED//-/_}"; shift ;;
        --) shift ; break ;;
...
NODE_ARCH=$(os-apply-config -m $DATAFILE --key arch)
case $NODE_ARCH in
...
: ${OVERCLOUD_CONTROL_DIB_EXTRA_ARGS:=rabbitmq-server}
...
do
    ...
    if [ "$i" == "OVERCLOUD-CONTROL" ]; then
       IMAGE_ELEMENTS="$SSL_ELEMENT $IMAGE_ELEMENTS"
    fi
