...
| .[0]
| .bootstack += {"public_interface_ip": ($bm_seed_ip + "/'"${BM_NETWORK_CIDR##*/}"'"),
                 "masquerade_networks": ($config["baremetal-network"].cidr // "192.0.2.0/24")
                }
| .heat |= (
...
| . + {"local-ipv4": $bm_seed_ip,
       "completion-signal": ("http://" + $config["host-ip"] + ":'"${SEED_COMP_PORT:=27414}"'"),
       "instance-id": "'"${SEED_IMAGE_ID:=seedImageID}"'"
      }
