...
# - nova.compute_driver to ironic.nova.virt.ironic.driver.IronicDriver
    jq -s '.[1] as $config |(.[0].ironic |= (.virtual_power_ssh_key=$config["ssh-key"]))|.[0].nova.compute_driver="ironic.nova.virt.ironic.driver.IronicDriver"| .[0]' config.json $TE_DATAFILE > local.json
fi
### --end
