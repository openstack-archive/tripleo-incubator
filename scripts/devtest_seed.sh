...
if [ $USE_IRONIC -eq 0 ]; then
    ...
    # until ironic can support nodes with ssh keys and not just a path to a key  https://review.openstack.org/#/c/80376
    ssh root@192.0.2.1 "cat /mnt/state/opt/stack/boot-stack/virtual-power-key.pub" > ~/.ssh/authorized_keys
fi
