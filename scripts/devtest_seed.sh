...
# Wait for network
wait_for -w --timeout 10 --delay 1 -- ping -c 1 $BM_NETWORK_SEED_IP
...
wait_for -w --timeout 500 --delay 10 -- nova list
user-config
...
wait_for -w --timeout 300 --delay 10 -- neutron agent-list -f csv -c alive -c agent_type -c host \| grep "\":-).*Open vSwitch agent.*\"" #nodocs
