...
class Stack(object):
    def __init__(self, name, master, template, inputs, outputs, env):
        self.name = name
        self.master = master
    ...
    def __repr__(self):
        return (
            'Stack(%(id)s, %(master), %(template)s, %(env)s)'
            % self.__dict__)
    ...
    for name, res in iter(doc['resources'].items()):
        ...
        if env and os.path.exists(env):
            with open(env) as env_file:
                this_env = json.loads(env_file)
                new_env = dict(
                    [(ekey, edat) for ekey, edat in this_env if ekey in
                     allowed_params])
        else:
            new_env = {}
        s = Stack(
