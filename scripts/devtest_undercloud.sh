...
BM_NETWORK_CIDR=$(os-apply-config -m $TE_DATAFILE --kexy baremetal-network.cidr --type raw --key-default '192.0.2.0/24')
if [ -n "$VLAN_ID" ]; then
...
setup-neutron $BM_NETWORK_UNDERCLOUD_RANGE_START $BM_NETWORK_UNDERCLOUD_RANGE_END $BM_NETWORK_CIDR $BM_NETWORK_GATEWAY $UNDERCLOUD_CTL_IP ctlplane $UNDERCLOUD_NAMESERVER
if [ -n "$VLAN_ID" ]; then
    ...
    setup-neutron $BM_VLAN_START $BM_VLAN_END $BM_VLAN_CIDR $VLAN_GW "" ctlplane $UNDERCLOUD_NAMESERVER "" "" "" $VLAN_ID public
fi
