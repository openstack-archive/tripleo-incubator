...
if [ "$USE_IRONIC" -eq 0 ] ; then
    ...
    HEAT_UNDERCLOUD_EXTRA_OPTS="-P PowerSSHHost=${POWER_HOST} -P PowerManager=${POWER_MANAGER} -P PowerUserName=${POWER_USER} -P PowerSSHPrivateKey=${POWER_KEY}"
    REGISTER_SERVICE_OPTS=""
    if [[ "$(jq '[.nodes[0].pm_type]' $TE_DATAFILE)" == *ipmi* ]] ; then
    ...
    if [[ "$(jq '[.nodes[0].pm_type]' $TE_DATAFILE)" == *ipmi* ]] ; then
        ...
        HEAT_UNDERCLOUD_EXTRA_OPTS="-P IronicPassword=${UNDERCLOUD_IRONIC_PASSWORD} -P PowerSSHPrivateKey=${POWER_KEY}"
    fi
fi
