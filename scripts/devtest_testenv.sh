...
TEMP=$(getopt -o h,n:,b: -l bridge-seed-to-interface:,nodes: -n $SCRIPT_NAME -- "$@")
...
while true ; do
    case "$1" in
        --bridge-seed-to-interface) BRIDGE_INTERFACE="--bridge-to-interface $2"; shift 2;;
...
##    where ethX is an otherwise unused interface plugged into the same LAN as
