...
### --end
## NOTE(gfidente): the following is meant to keep some compatibility with old
## workflows where people intentionally left the TRIPLEO_ROOT var unset
if [ -z "$TRIPLEO_ROOT" -a -d ~/.cache/tripleo ]; then
  echo "WARNING: Defaulting TRIPLEO_ROOT to ~/.cache/tripleo"
  TRIPLEO_ROOT=~/.cache/tripleo
fi

## NOTE(gfidente): at this stage TRIPLEO_ROOT should never be found empty
