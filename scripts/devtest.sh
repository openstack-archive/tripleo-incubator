...
while true ; do
    case "$1" in
        ...
        # For a parallel build we mustn't use the local apt cache as it gets locked...
        --parallel-build) PARALLEL_BUILD=--parallel-build; export DIB_APT_LOCAL_CACHE=0; shift 1;;
...
# Create 2 vars needed for holding information on subprocesses, filled by background_build
new_pids=
declare -a pid_names
background_build devtest_ramdisk.sh
...
background_build devtest_seed.sh $BUILD_ONLY $ALLNODES
