...
function show_options () {
    ...
    echo "    --without-mergepy      -- Use the standalone Heat templates."
...
TEMP=$(getopt -o h,c -l build-only,without-mergepy,debug-logging,existing-environment,help,trash-my-machine,nodes:,bm-networks:,no-undercloud,heat-env-overcloud:,heat-env-undercloud: -n $SCRIPT_NAME -- "$@")
...
while true ; do
    case "$1" in
        ...
        --without-mergepy) WITHOUT_MERGEPY=--without-mergepy; shift 1;;
...
devtest_overcloud.sh $BUILD_ONLY $WITHOUT_MERGEPY $DEBUG_LOGGING $HEAT_ENV_OVERCLOUD
