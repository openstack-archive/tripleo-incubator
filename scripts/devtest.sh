...
DEVTEST_PERF_LOG="${SCRIPT_HOME}/devtest_perf.log" #nodocs
exec 3>>${DEVTEST_PERF_LOG} #nodocs
TIMESTAMP=$(date "+[%Y-%m-%d %H:%M:%S]") #nodocs
echo "${TIMESTAMP} Total runtime: $((${DEVTEST_END}-${DEVTEST_START})) s" | tee /dev/fd/3 #nodocs
echo "${TIMESTAMP}  ramdisk    : $((${DEVTEST_RD_END}-${DEVTEST_RD_START})) s" | tee /dev/fd/3 #nodocs
echo "${TIMESTAMP}  seed       : $((${DEVTEST_SD_END}-${DEVTEST_SD_START})) s" | tee /dev/fd/3 #nodocs
echo "${TIMESTAMP}  undercloud : $((${DEVTEST_UC_END}-${DEVTEST_UC_START})) s" | tee /dev/fd/3 #nodocs
echo "${TIMESTAMP}  overcloud  : $((${DEVTEST_OC_END}-${DEVTEST_OC_START})) s" | tee /dev/fd/3 #nodocs
echo "${TIMESTAMP} DIB_COMMON_ELEMENTS=${DIB_COMMON_ELEMENTS}" >&3 #nodocs
