...
if [ "$USE_CONTAINERS" == 1 ]; then
    ...
    glance image-create --name fedora-atomic --file $ATOMIC_IMAGE_NAME \
        --disk-format qcow2 --container-format bare
    export RESOURCE_REGISTRY_PATH="$TRIPLEO_ROOT/tripleo-heat-templates/overcloud-resource-registry-docker.yaml"
    DISK_IMAGES_CONFIG="$TRIPLEO_ROOT/tripleo-incubator/scripts/overcloud_disk_images_docker.yaml"
    CONTAINER_ARGS='--compute-flavor=baremetal_fulldisk --disk-images-config=$DISK_IMAGES_CONFIG'
fi
