...
if [ "$TRIPLEO_CLEANUP" = "1" -a -z "$BUILD_ONLY" ]; then #nodocs
...
if [ -z "$NO_UNDERCLOUD" ]; then
  ...
  test -z $BUILD_ONLY && export no_proxy=$no_proxy,$(os-apply-config --type raw -m $TE_DATAFILE --key undercloud.endpointhost)
  test -z $BUILD_ONLY && source $TRIPLEO_ROOT/tripleo-incubator/undercloudrc
fi
...
test -z $BUILD_ONLY && export no_proxy=$no_proxy,$(os-apply-config --type raw -m $TE_DATAFILE --key overcloud.endpointhost)
test -z $BUILD_ONLY && source $TRIPLEO_ROOT/tripleo-incubator/overcloudrc
